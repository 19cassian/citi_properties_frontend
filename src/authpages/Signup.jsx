...
function PasswordRequirement({ met, children }) {
  const { theme } = useTheme();
  return (
    <li className={`flex items-center gap-1.5 ${met ? "text-emerald-500" : theme.textMuted}`}>
      {met ? <CircleCheck className="h-3.5 w-3.5" /> : <Circle className="h-3.5 w-3.5" />}
      {children}
    </li>
  );
}
...
function RegisterForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const values = { first_name, last_name, email, username, password, role };
    ...
    <div className="flex min-h-full w-full flex-col justify-center px-5 py-10 sm:px-10 lg:px-16 xl:px-20">
      <div className="mx-auto w-full max-w-[420px]">
        ...
        <form className="mt-7 space-y-4" onSubmit={handleSubmit} noValidate>
          ...
          <Field label="Password" htmlFor="password">
            ...
            <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs">
              <PasswordRequirement met={pwChecks.length}>8+ characters</PasswordRequirement>
              <PasswordRequirement met={pwChecks.upper}>One uppercase letter</PasswordRequirement>
              <PasswordRequirement met={pwChecks.number}>One number</PasswordRequirement>
            </ul>
          </Field>
          ...
          <div>
            <label className="flex items-start gap-2.5 text-sm">
              <input
                ...
                className="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500/30 dark:border-slate-700"
              />
              <span className={theme.textBody}>
                I agree to the <a href="#" className="font-medium text-indigo-500 hover:underline">Terms of Service</a> and <a href="#" className="font-medium text-indigo-500 hover:underline">Privacy Policy</a>.
              </span>
            </label>
        ...
        <p className={`mt-6 text-center text-sm ${theme.textBody}`}>
          ...
          <a onClick={() => navigate("/login")} href="#" className="font-medium text-indigo-500 hover:underline">Log in</a>
        </p>
